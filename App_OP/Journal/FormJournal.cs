...
namespace App_OP
{
    public partial class FormJournal : BaseForm
    {
        ...
        private void FormJournal_Shown(object sender, EventArgs e)
        {
            InitUI();
        }
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            var sql = $@"
select * from (
select * from op_journal where ProcessTime>='{this.StartTime.Value.Start()}' and ProcessTime<='{this.EndTime.Value.End()}' and DoctorNo = '{SysContext.CurrUser.user.Code}'
union all
select * from op_journal_history where ProcessTime>='{this.StartTime.Value.Start()}' and ProcessTime<='{this.EndTime.Value.End()}' and DoctorNo = '{SysContext.CurrUser.user.Code}') a order by ProcessTime desc
";
        ...
        private void SearchDiagnosis()
        {
            ...
            while (true)
            {
                if (skip >= treatmentNos.Count)
                    break;
                var tmp = treatmentNos.Skip(skip).Take(200);
                skip += 200;
