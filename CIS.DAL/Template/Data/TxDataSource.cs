...
namespace CIS.DAL.Template
{
    ...
    public class TxDataSource
    {
        ...
        /// 加载数据源
        /// </summary>
        public static TxDataSource Load()
        {
            string dataSourceConfig = CIS.Purview.SysParameterDal.Get("DataSourceConfig");
            if (string.IsNullOrEmpty(dataSourceConfig))
                return new TxDataSource();
            try
            {
                return Utility.XMLHelper.LoadObjectFromXMLString(typeof(TxDataSource), dataSourceConfig) as TxDataSource;
            }
            catch
            {
                return new TxDataSource();
        ...
        public void Save()
        {
            if (CIS.Purview.SysParameterDal.Exists("DataSourceConfig"))
                CIS.Purview.SysParameterDal.Update("DataSourceConfig", this.ToXML());
            else
                CIS.Purview.SysParameterDal.Add("DataSourceConfig", "数据源配置", "数据源配置XML", this.ToXML());

        }
        public string ToXML()
        {
            return Utility.XMLHelper.SaveObjectToXMLString(this);
        }
        /// <summary>
        /// 通过类型注册数据源节点
        /// </summary>
